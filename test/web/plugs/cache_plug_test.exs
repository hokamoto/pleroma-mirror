...
defmodule Pleroma.Web.CachePlugTest do
  use ExUnit.Case, async: true
  use Plug.Test

  alias Pleroma.Web.CachePlug
  ...
  test "caches a response" do
    assert @miss_resp ==
             conn(:get, "/")
             |> CachePlug.call(%{query_params: false, ttl: nil})
  ...
  test "ttl is set" do
    assert @miss_resp ==
             conn(:get, "/")
             |> CachePlug.call(%{query_params: false, ttl: @ttl})
             |> put_resp_content_type("cofe/hot")
             |> send_resp(:ok, "cofe")
             |> sent_resp()
  ...
  test "take query string into account when `query_params` is true" do
    assert @miss_resp ==
             conn(:get, "/?cofe")
             |> CachePlug.call(%{query_params: true, ttl: nil})
             |> put_resp_content_type("cofe/hot")
             |> send_resp(:ok, "cofe")
             |> sent_resp()
  ...
  test "take specific query params into account when `query_params` is list" do
    assert @miss_resp ==
             ...
             |> fetch_query_params()
             |> CachePlug.call(%{query_params: ["a", "b", "c"], ttl: nil})
             |> put_resp_content_type("cofe/hot")
             |> send_resp(:ok, "cofe")
             |> sent_resp()

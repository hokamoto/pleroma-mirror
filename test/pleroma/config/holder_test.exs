...
defmodule Pleroma.Config.HolderTest do
  ...
  test "config/1" do
    ...
    assert pleroma_config[Pleroma.Upload.Uploaders.Local][:uploads] == "test/uploads"
    tesla_config = Holder.config(:tesla)
    assert tesla_config[:adapter] == Tesla.Mock
  end

  test "config/2" do
    assert Holder.config(:pleroma, Pleroma.Upload.Uploaders.Local) == [uploads: "test/uploads"]

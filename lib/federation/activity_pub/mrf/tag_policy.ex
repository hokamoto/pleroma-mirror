...
defmodule Pleroma.Federation.ActivityPub.MRF.TagPolicy do
  alias Pleroma.User
  ...
  defp process_tag(
    ...
    if Enum.member?(to, Constants.as_public()) do
      to = List.delete(to, Constants.as_public()) ++ [user.follower_address]
      cc = List.delete(cc, user.follower_address) ++ [Constants.as_public()]
  ...
  defp process_tag(
    ...
    if Enum.member?(to, Constants.as_public()) or
         Enum.member?(cc, Constants.as_public()) do
      to = List.delete(to, Constants.as_public()) ++ [user.follower_address]
      cc = List.delete(cc, Constants.as_public())

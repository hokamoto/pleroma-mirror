...
defmodule Pleroma.Web.RateLimiterPlugTest do
  ...
  describe "config" do
    test "config is required for plug to work" do
      ...
      assert %{limits: {1, 1}, name: :test_init, opts: [name: :test_init]} ==
               RateLimiterPlug.init(name: limiter_name)

      assert nil == RateLimiterPlug.init(name: :foo)
    end
    ...
    test "it is disabled for localhost" do
      ...
      Pleroma.Config.put([Pleroma.Web.RemoteIPPlug, :enabled], false)
    ...
    test "it restricts based on config values" do
      ...
      for i <- 1..5 do
        ...
        assert {^i, _} = RateLimiterPlug.inspect_bucket(conn, limiter_name, opts)
        Process.sleep(10)
      end
      ...
      conn = RateLimiterPlug.call(conn, opts)
      assert {1, 4} = RateLimiterPlug.inspect_bucket(conn, limiter_name, opts)
  ...
  describe "options" do
    test "`bucket_name` option overrides default bucket name" do
      ...
      opts = RateLimiterPlug.init(name: limiter_name, bucket_name: base_bucket_name)

      conn = conn(:get, "/")

      RateLimiterPlug.call(conn, opts)
      assert {1, 4} = RateLimiterPlug.inspect_bucket(conn, base_bucket_name, opts)
      assert {:err, :not_found} = RateLimiterPlug.inspect_bucket(conn, limiter_name, opts)
    end
    ...
    test "`params` option allows different queries to be tracked independently" do
      ...
      opts = RateLimiterPlug.init(name: limiter_name, params: ["id"])
      ...
      RateLimiterPlug.call(conn, opts)
      assert {1, 4} = RateLimiterPlug.inspect_bucket(conn, limiter_name, opts)
      assert {0, 5} = RateLimiterPlug.inspect_bucket(conn_2, limiter_name, opts)
    end
    ...
    test "it supports combination of options modifying bucket name" do
      ...
      RateLimiterPlug.call(conn, opts)
      assert {1, 4} = RateLimiterPlug.inspect_bucket(conn, base_bucket_name, opts)
      assert {0, 5} = RateLimiterPlug.inspect_bucket(conn_2, base_bucket_name, opts)
    end
  end
  ...
  describe "unauthenticated users" do
    test "are restricted based on remote IP" do
      ...
      conn_2 = RateLimiterPlug.call(conn_2, opts)
      assert {1, 4} = RateLimiterPlug.inspect_bucket(conn_2, limiter_name, opts)
